...
TEMPLATE_CLOUD_SYNC_ERROR = os.path.join(FREEZE_SCRIPT_DIR, "云同步出错.png")
...
def _monitor_cloud_sync_error_after_start():
    """点击开始游戏后后台监控30秒；检测到云同步出错提示时点击一次。"""
    if not os.path.exists(TEMPLATE_CLOUD_SYNC_ERROR):
        print(f"[启动监控] 云同步出错模板不存在，跳过检测: {TEMPLATE_CLOUD_SYNC_ERROR}")
        return

    print("[启动监控] 开始并行检测云同步出错提示（持续30秒）...")
    deadline = time.monotonic() + 30.0
    while not STOP_FLAG and time.monotonic() < deadline:
        error_pos = _find_image_on_screen(
            TEMPLATE_CLOUD_SYNC_ERROR,
            threshold=FREEZE_MATCH_THRESHOLD,
        )
        if error_pos:
            print(
                f"[启动监控] 检测到云同步出错提示，点击位置: "
                f"({error_pos[0]}, {error_pos[1]})"
            )
            _click_at(error_pos[0], error_pos[1], delay=0.5)
            return
        time.sleep(1.0)
    print("[启动监控] 30秒内未检测到云同步出错提示，继续正常流程")


def _start_cloud_sync_error_monitor():
    """以守护线程启动云同步出错监控，不阻塞游戏启动主流程。"""
    monitor_thread = threading.Thread(
        target=_monitor_cloud_sync_error_after_start,
        name="cloud-sync-error-monitor",
        daemon=True,
    )
...
def _click_start_game_if_available(log_prefix):
    """检测并点击已经出现的开始游戏按钮。"""
    start_pos = _find_image_on_screen(
        TEMPLATE_START_GAME,
        threshold=FREEZE_MATCH_THRESHOLD,
    )
    if not start_pos:
        return False
    print(
        f"{log_prefix} 找到'开始游戏'按钮，位置: "
        f"({start_pos[0]}, {start_pos[1]})，直接继续启动"
    )
    _click_at(start_pos[0], start_pos[1], delay=0.5)
    _start_cloud_sync_error_monitor()
    return True


def _wait_for_dd2_window_after_start(log_prefix):
    """点击开始游戏后等待DD2窗口出现。"""
    print(f"{log_prefix} 等待DD2游戏窗口出现...")
    for attempt in range(60):
        if STOP_FLAG:
            return None
        hwnd = win32gui.FindWindow(CONFIG["game_class"], CONFIG["game_title"])
        if hwnd:
            print(f"{log_prefix} DD2游戏窗口已出现，句柄: {hwnd}")
            focus_game_window(hwnd)
            time.sleep(3)
            return hwnd
        print(f"{log_prefix} 等待游戏窗口... {attempt + 1}/60")
        time.sleep(5)
    return None


def _cancel_stuck_steam_launch_and_wait(log_prefix):
    """取消卡住的启动；期间开始游戏一旦出现便立即点击。"""
    print(
        f"{log_prefix} 30次未找到'开始游戏'，"
        "同时查找'开始游戏'和'取消'按钮..."
    )
    cancel_pos = None
    for attempt in range(15):
        if STOP_FLAG:
            return "stopped"
        if _click_start_game_if_available(log_prefix):
            return "started"
        cancel_pos = _find_image_on_screen(
            TEMPLATE_CANCEL,
            threshold=CANCEL_MATCH_THRESHOLD,
        )
        if cancel_pos:
            print(
                f"{log_prefix} 找到'取消'按钮，位置: "
                f"({cancel_pos[0]}, {cancel_pos[1]})"
            )
            _click_at(cancel_pos[0], cancel_pos[1], delay=0.5)
            time.sleep(2)
            break
        print(f"{log_prefix} 未找到'取消'按钮，重试 {attempt + 1}/15...")
        time.sleep(1)

    if not cancel_pos:
        print(f"{log_prefix} 未找到'取消'按钮，继续检查是否已有确认弹窗")

    print(f"{log_prefix} 同时查找'开始游戏'和'确认'按钮...")
    confirm_pos = None
    for attempt in range(15):
        if STOP_FLAG:
            return "stopped"
        if _click_start_game_if_available(log_prefix):
            return "started"
        confirm_pos = _find_image_on_screen(
            TEMPLATE_CONFIRM,
            threshold=FREEZE_MATCH_THRESHOLD,
        )
        if confirm_pos:
            print(
                f"{log_prefix} 找到'确认'按钮，位置: "
                f"({confirm_pos[0]}, {confirm_pos[1]})"
            )
            _click_at(confirm_pos[0], confirm_pos[1], delay=0.5)
            break
        print(f"{log_prefix} 未找到'确认'按钮，重试 {attempt + 1}/15...")
        time.sleep(1)

    if not confirm_pos:
        print(f"{log_prefix} 未找到'确认'按钮，仍等待Steam自行恢复")

    print(
        f"{log_prefix} 最多等待60秒；期间'开始游戏'出现就立即点击，"
        "否则重新走Steam流程..."
    )
    for remaining in range(60, 0, -1):
        ...
        if remaining in (60, 30, 10):
            print(f"{log_prefix} 距离重新启动还有 {remaining} 秒")
        time.sleep(1)
    return "relaunch"


def _launch_game_via_steam_until_window(log_prefix):
    """持续通过Steam启动游戏，直到DD2窗口出现或收到停止信号。"""
    launch_attempt = 0
    while not STOP_FLAG:
        launch_attempt += 1
        print(f"{log_prefix} 重新走Steam启动流程，第 {launch_attempt} 次...")
        _press_win_key()

        screen_w, screen_h = pyautogui.size()
        left_region = (0, 0, screen_w // 2, screen_h)
        steam_pos = None
        for attempt in range(15):
            if STOP_FLAG:
                return None
            steam_pos = _find_image_on_screen(
                TEMPLATE_STEAM,
                threshold=0.6,
                region=left_region,
            )
            if steam_pos:
                print(f"{log_prefix} 找到Steam图标，位置: ({steam_pos[0]}, {steam_pos[1]})")
                _click_at(steam_pos[0], steam_pos[1], delay=0.5)
                break
            print(f"{log_prefix} 未找到Steam图标，重试 {attempt + 1}/15...")
            time.sleep(1)
        if not steam_pos:
            print(f"{log_prefix} 本轮未找到Steam图标，重新开始Steam启动流程")
            continue

        time.sleep(3)

        # 若Steam仍认为游戏在运行，按原卡死流程先点停止并确认。
        stop_pos = _find_image_on_screen(
            TEMPLATE_STOP,
            threshold=FREEZE_MATCH_THRESHOLD,
        )
        if stop_pos:
            print(f"{log_prefix} 找到'停止'按钮，按卡死流程停止当前游戏")
            ...
            for attempt in range(15):
                ...
                if confirm_pos:
                    print(f"{log_prefix} 找到'确认'按钮，确认停止")
                    _click_at(confirm_pos[0], confirm_pos[1], delay=0.5)
                    break
                print(f"{log_prefix} 未找到'确认'按钮，重试 {attempt + 1}/15...")
                time.sleep(1)
            if not confirm_pos:
                print(f"{log_prefix} 未找到'确认'按钮，重新开始Steam启动流程")
                continue
            time.sleep(3)

        print(f"{log_prefix} 查找'开始游戏'按钮...")
        start_pos = None
        ...
        for attempt in range(30):
            ...
            if start_pos:
                print(f"{log_prefix} 找到'开始游戏'按钮，位置: ({start_pos[0]}, {start_pos[1]})")
                _click_at(start_pos[0], start_pos[1], delay=0.5)
                _start_cloud_sync_error_monitor()
                break
            if attempt >= 9 and not game_end_clicked:
                game_end_pos = _find_image_on_screen(
                    TEMPLATE_GAME_END,
                    threshold=FREEZE_MATCH_THRESHOLD,
                )
                if game_end_pos:
                    print(
                        f"{log_prefix} 连续10次未找到'开始游戏'，检测到 gameend，"
                        f"点击位置: ({game_end_pos[0]}, {game_end_pos[1]})"
                    )
                    _click_at(game_end_pos[0], game_end_pos[1], delay=0.5)
                    game_end_clicked = True
                    time.sleep(2)
                    continue
            print(f"{log_prefix} 未找到'开始游戏'按钮，重试 {attempt + 1}/30...")
            time.sleep(2)

        if not start_pos:
            timeout_result = _cancel_stuck_steam_launch_and_wait(log_prefix)
            if timeout_result == "stopped":
                return None
            if timeout_result == "relaunch":
                continue

        hwnd = _wait_for_dd2_window_after_start(log_prefix)
        if hwnd:
            return hwnd

        print(f"{log_prefix} 等待游戏窗口超时，重新走Steam启动流程")
        for _ in range(10):
            setfaild_pos = _find_image_on_screen(
                CONFIG["setfaild_template"],
                threshold=FREEZE_MATCH_THRESHOLD,
            )
            if setfaild_pos:
                print(f"{log_prefix} 检测到setfaild，点击关闭")
...
def _handle_start_game_timeout_and_relaunch(log_prefix):
    """连续30次未命中后处理取消/确认；开始游戏出现时直接继续。"""
    timeout_result = _cancel_stuck_steam_launch_and_wait(log_prefix)
    if timeout_result == "stopped":
        return None
    if timeout_result == "started":
        hwnd = _wait_for_dd2_window_after_start(log_prefix)
        if hwnd:
            return hwnd
        print(f"{log_prefix} 点击开始游戏后等待窗口超时，重新走Steam启动流程")
    return _launch_game_via_steam_until_window(log_prefix)
...
def recover_game():
    ...
    if not start_pos:
        print("[恢复] 30次未找到'开始游戏'按钮，不停止脚本")
        retry_hwnd = _handle_start_game_timeout_and_relaunch("[恢复]")
        if not retry_hwnd:
            return False
    ...
    try:
        ...
        if hwnd:
            ...
            for launch_attempt in range(3):
                ...
                if not start_pos:
                    print("[INFO] 30次未找到'开始游戏'按钮，不停止脚本")
                    hwnd = _handle_start_game_timeout_and_relaunch("[INFO]")
                    if hwnd:
                        launched = True
                        break
                    if STOP_FLAG:
